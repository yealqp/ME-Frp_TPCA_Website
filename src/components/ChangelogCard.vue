<template>
  <div class="changelog-card bg-white/5 rounded-lg p-5 hover:bg-white/10 transition-smooth border border-white/10"
    ...
    <div class="flex flex-wrap items-center gap-3 mb-3">
      ...
      <div class="flex flex-wrap items-center gap-2">
        <span v-if="update.isLatest"
          ...
          <UIcon name="i-lucide-sparkles" class="size-3 inline-block mr-1" />
          最新版本
        </span>
        ...
        <span v-if="update.date"
          ...
          <UIcon name="i-lucide-calendar" class="size-3 inline-block mr-1" />
          {{ update.date }}
        </span>
      </div>
    </div>
    ...
    <!-- 版本描述 -->
    <p v-if="update.description || update.note" class="text-gray-400 text-sm mb-4 italic"
      v-html="update.description || update.note"></p>
    ...
    <div class="space-y-2">
      ...
      <ul class="space-y-2">
        <li v-for="(item, index) in changeList" :key="index"
          ...
          <span
            class="flex-shrink-0 w-1.5 h-1.5 bg-primary-400 rounded-full mt-2 group-hover:scale-125 transition-transform"></span>
          <span class="flex-1 leading-relaxed" v-html="formatChangeItem(item)"></span>
        </li>
      </ul>
    </div>
  </div>
